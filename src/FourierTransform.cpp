...
#pragma omp parallel for default(none) shared(omega, m)
    for (size_t j = 0; j < m / 2; j++)
      omega[j] = std::exp(std::complex<real>{0, (-2 * pi / m) * j});

#pragma omp parallel for default(none) firstprivate(m, n) \
    shared(result, omega) schedule(static) collapse(2)
    for (size_t k = 0; k < n; k += m) {
      for (size_t j = 0; j < m / 2; j++) {
        const std::complex<real> t = omega[j] * result[k + j + m / 2];
