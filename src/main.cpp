...
int main(int argc, char* argv[]) {
  ...
  // Perform different code based on whether Fourier or wavelet transforms are
  // used.
  if (mode != std::string("waveletTest")) {
    ...
    if (mode == std::string("demo")) {
      ...
      if (!CompareVectors(input_sequence, classical_ift_result, precision,
                          false))
        std::cerr << "Errors detected in classical inverse FFT." << std::endl;
      if (!CompareVectors(input_sequence, iterative_ift_result, precision,
                          false))
